...
if [[ "$first_line" != "$expected_shebang" ]]; then
    printf '{"systemMessage": "💡 PEP 723: Python script shebang should be `#!/usr/bin/env -S uv run --script` (found: %s). Please Edit the file to fix the shebang and add a PEP 723 metadata block."}\n' "$first_line"
    exit 0
fi

# Check 2: PEP 723 metadata block must exist
if ! printf '%s' "$content" | grep -qF '# /// script'; then
    printf '{"systemMessage": "💡 PEP 723: Script has the correct shebang but is missing the inline metadata block. Please Edit the file to add after the shebang:\n# /// script\n# requires-python = \">=3.11\"\n# dependencies = [\n#   \"package-name\",\n# ]\n# ///"}\n'
    exit 0
fi
