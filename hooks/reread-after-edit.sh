...
case "$file_path" in
    *.md|*.py|*.ts|*.js|*.tsx|*.jsx|*.c|*.cpp|*.h|*.hpp|*.rs|*.go|*.java|*.sh|*.toml|*.yaml|*.yml|*.json)
        printf 'Silently re-read the edited region of %s (±30 lines) and audit it. If clean, a terse "Done." is fine — do not narrate what you checked. If you find an issue, report it and fix it proactively in the same turn. See "Re-read After Edit" in CLAUDE.md.\n' "$file_path" >&2
        exit 2
        ;;
esac
